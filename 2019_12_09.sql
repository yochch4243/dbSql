...
--PRIMARY KEY : pk_테이블명
CREATE TABLE dept_test(
    deptno NUMBER(2) CONSTRAINT pk_dept_test PRIMARK KEY,;
    
    
--PAIRWISE : 쌍의 개념
--상단의 PRIMARY KEY 제약 조건의 경유 하나의 컬럼에 제약조건을 생성
--여러 컬럼을 복합으로 PRIMARY KEY 제약으로 생성 할 수있다
--해당 방법은 위의 두가지 예시처럼 컬럼레벨에서는 생성 할 수 없다.
--> TABLE LEVEL 제약 조건생성
...
--loc 컬럼은 nullable이기 때문에 null값이 입력 될 수 있다.
INSERT INTO dept_test VALUES(99, 'ddit', NULL);


--deptno 컬럼이 primary key 제약에 걸리지 않고(중복된 값이 아니기 때문에)
...
--dname컬럼에 적용된 UNIQUE 제약에 의해 두번째 쿼리는 정상적으로 실행될 수 없다
INSERT INTO dept_test VALUES(99, 'ddit', 'daejeon');
...
--ON DELETE CASCADE옵션에 따라 DEPT 데이터를 삭제할 경우
--해당 데이터를 참조하고 있는 EMP테이블의 사원 데이터도 함께 삭제된다.
DELETE dept_test
WHERE deptno = 99;
ROLLBACK;
...
--ON DELETE SET NULL옵션에 따라 DEPT 데이터를 삭제할 경우
--해당 데이터를 참조하고 있는 EMP테이블의 DEPTNO컬럼을 NULL로 변경
DELETE dept_test
WHERE deptno = 99;
ROLLBACK;
...
--empno NUMBER(4)
--ename VARCHAR2(10)
--sal NUMBER(7, 2) --01보다 큰 숫자만 입력되도록 제한
...
--check 제약조건에 위배되지 않으므로 정상입력 (sal, emp_gb)
...
--현재 emp_test테이블에 데이터가 없기 때문에 컬럼타입을 변경하는 것이 가능하다
...
--emp_test 테이블의 PTIMARY KEY 제약조건은 pk_emp_test 제약 삭제
