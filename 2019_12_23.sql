...
SET SERVEROUTPUT ON;
DECLARE
    -- dept테이블의 row정보를 담을 수 있는 ROWTYPE 변수선언
...
SET SERVEROUTPUT ON;
DECLARE
    -- 부서이름, LOC정보를 저장할 수 있는 RECORD TYPE 선언
    TYPE dept_row IS RECORD(
        dname dept.dname%TYPE,
        loc dept.loc%TYPE);
    --type 선언이 완료, type을 가지고 변수를 생성
    --java : class 생성 후 해당 class의 인스턴스를 생성(new)
    --pl/ sql 변수생성 : 변수이름 변수타입 dname dept.dname%TYPE;
...
-- col --> row --> table
-- TYPE 테이블타입명 IS TABLE OF ROWTYPE/RECORD INDEX BY 인덱스 타입(BINARY_INTGER)
...
-- arr(1).name = 'brown'
-- arr('brown').name = 'brown'

-- dept테이블의 row를 여러 건 저장할 수 있는 dept_tab TABLE TYPE 선언하여
-- SELECT * FROM dept;의 결과 여러건을 변수에 담는다.
DECLARE
...
-- 명시적 : 개발자가 이름을 붙인 커서. 개발자가 직접 제어하며 선언, OPEN, FETCH, CLOSE단계가 존재
...
-- 부서 테이블의 모든행의 부서이름, 위치 지역 정보를 출력 (CURSOR를 이용)
DECLARE
    --커서 선언
